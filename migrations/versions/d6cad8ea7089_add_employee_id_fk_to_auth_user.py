"""Add employee_id FK to auth.user

Revision ID: d6cad8ea7089
Revises: 800241085ded
Create Date: 2026-02-27 11:27:12.826142
...
revision = 'd6cad8ea7089'
down_revision = '800241085ded'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    sa.Column('is_active', sa.Boolean(), nullable=False),
    sa.Column('requirements_status', sa.String(length=20), nullable=True),
    sa.Column('created_at', sa.DateTime(), server_default=sa.text('SYSUTCDATETIME()'), nullable=False),
    sa.Column('updated_at', sa.DateTime(), server_default=sa.text('SYSUTCDATETIME()'), nullable=False),
    ...
    with op.batch_alter_table('employee', schema='org') as batch_op:
        batch_op.create_index(batch_op.f('ix_org_employee_employee_code'), ['employee_code'], unique=True)
    ...
    with op.batch_alter_table('user', schema='auth') as batch_op:
        batch_op.create_index(batch_op.f('ix_auth_user_employee_id'), ['employee_id'], unique=True)
...
def downgrade():
    ...
    with op.batch_alter_table('user', schema='auth') as batch_op:
        batch_op.drop_index(batch_op.f('ix_auth_user_employee_id'))
    ...
    with op.batch_alter_table('employee', schema='org') as batch_op:
        ...
        batch_op.drop_index(batch_op.f('ix_org_employee_employee_code'))
