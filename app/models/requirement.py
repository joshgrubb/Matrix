...
These junction tables define what hardware types and software products
...
class PositionHardware(db.Model):
    """
    Defines what hardware types a position requires and in what quantity.
    ...
    __table_args__ = (
        # Each position can only have one entry per hardware type.
        db.UniqueConstraint(
            "position_id",
            "hardware_type_id",
            name="UQ_position_hardware_position_type",
        ),
        {"schema": "equip"},
    )
    ...
    position = db.relationship(
        "Position", back_populates="hardware_requirements"
    )
    hardware_type = db.relationship(
        "HardwareType", back_populates="position_hardware"
    )

    def __repr__(self) -> str:
        return (
            f"<PositionHardware position={self.position_id} "
            f"hw_type={self.hardware_type_id} qty={self.quantity}>"
        )
...
class PositionSoftware(db.Model):
    ...
    # -- Relationships -----------------------------------------------------
    position = db.relationship(
        "Position", back_populates="software_requirements"
    )
    software = db.relationship(
        "Software", back_populates="position_software"
    )
