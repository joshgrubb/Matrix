...
class HardwareTypeCostHistory(db.Model):
    ...
    def __repr__(self) -> str:
        return (
            f"<HWCostHistory hw={self.hardware_type_id} "
...
class CostSnapshot(db.Model):
    """
    Point-in-time snapshot of calculated costs for reporting.

    Allows historical cost comparison without recalculation.
    ``snapshot_type`` values: department, division, position.
    ``cost_data`` stores the full cost breakdown as JSON.
    """
    ...
    cost_data = db.Column(db.Text, nullable=False)
    ...
    def __repr__(self) -> str:
        return (
            f"<CostSnapshot {self.snapshot_type}:{self.entity_id} "
