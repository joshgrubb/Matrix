"""
Seed script — create a development manager user for local testing.

Registers a ``flask seed-dev-manager`` CLI command that creates (or
reactivates) a local manager user with organization-wide scope.  This
user is used with the ``/auth/dev-login`` bypass route so you can
develop and test without a working Entra ID app registration.

Usage::

    flask seed-dev-manager                    # Create with defaults
    flask seed-dev-manager --email me@co.gov  # Custom email
    flask seed-dev-manager --first Dev        # Custom first name
    flask seed-dev-manager --last manager       # Custom last name

Prerequisites:
    - The database must exist and the DDL script must have been run.
    - The ``auth.role`` table must contain seed data (at minimum the
      ``manager`` role).  If roles haven't been seeded yet, this script
...
# -- Default values for the dev manager user ---------------------------------
_DEFAULT_EMAIL = "dev.manager@localhost"
_DEFAULT_FIRST_NAME = "Dev"
_DEFAULT_LAST_NAME = "manager"


@click.command("seed-dev-manager")
@click.option(
    "--email",
    default=_DEFAULT_EMAIL,
    show_default=True,
    help="Email address for the dev manager user.",
)
@click.option(
    "--first",
    "first_name",
    ...
    show_default=True,
    help="First name for the dev manager user.",
)
@click.option(
    "--last",
    "last_name",
    ...
    show_default=True,
    help="Last name for the dev manager user.",
)
@with_appcontext
def seed_dev_manager_command(email: str, first_name: str, last_name: str):
    """
    Create a development manager user for local testing.

    If a user with the given email already exists, the script will
    ensure they have the manager role, are active, and have org-wide
    scope — then exit without creating a duplicate.
    """
    click.echo("=" * 60)
    click.echo("  PositionMatrix — Seed Dev manager User")
    click.echo("=" * 60)

    # -- Step 1: Verify the manager role exists -------------------------------
    click.echo("\n[1/3] Looking up manager role...")
    manager_role = Role.query.filter_by(role_name="manager").first()

    if manager_role is None:
        click.secho(
            "      ✗ The 'manager' role was not found in auth.role.",
            fg="red",
        )
    ...
    click.secho(
        f"      ✓ Found role: {manager_role.role_name} (id={manager_role.id})",
        fg="green",
    )

    # -- Step 2: Create or update the user ---------------------------------
    click.echo("\n[2/3] Creating dev manager user...")
    user = User.query.filter(User.email.ilike(email)).first()

    if user is not None:
        # User already exists — ensure they are manager and active.
        click.echo(f"      User '{email}' already exists (id={user.id}).")
        changed = False

        if user.role_id != manager_role.id:
            user.role_id = manager_role.id
            click.echo("      → Updated role to manager.")
            changed = True
        ...
        if changed:
            ...
            click.secho("      ✓ User is already an active manager.", fg="green")
    else:
    ...
    # -- Summary -----------------------------------------------------------
    click.echo("\n" + "=" * 60)
    click.secho("  Dev manager user is ready.", fg="green", bold=True)
...
def register_seed_commands(app):
    ...
    app.cli.add_command(seed_dev_manager_command)
